...
const townregEx = /\w+\:/
const monthregEx = /\w{3}\s/g

function mean(town, strng) {
    var dataArr = strng.split(/\n/)
    var townArr = dataArr.map(e => townregEx.exec(e)[0].replace(":", "")) //?
    var index = townArr.indexOf(town)
    if (index == -1) return -1;
    var amountArr = dataArr[index].replace(townregEx, "").replace(monthregEx, "").split(/\,/)
    return amountArr.reduce((prev, curr, i) => {
        prev += parseFloat(curr);
        if (i == amountArr.length - 1) prev = prev / amountArr.length
        return prev
    }, 0)
}
function variance(town, strng) {
    const meanVal = mean(town, strng)
    if (meanVal == -1) return -1 //?
    var dataArr = strng.split(/\n/)
    var townArr = dataArr.map(e => townregEx.exec(e)[0].replace(":", "")) //?
    var index = townArr.indexOf(town)
    var amountArr = dataArr[index].replace(townregEx, "").replace(monthregEx, "").split(/\,/)
    return amountArr.reduce((prev, curr, i) => {
        prev += Math.pow(parseFloat(curr) - meanVal, 2)
        if (i == amountArr.length - 1) prev = prev / amountArr.length
        return prev;
    }, 0)
}
...
// }
// Test.describe("rainfall",function() {
// Test.it("Basic tests: mean, data",function() {  
//     assertFuzzyEquals(mean("London", data), 51.199999999999996) 
//     assertFuzzyEquals(mean("Beijing", data), 52.416666666666664)
// })})
// Test.describe("rainfall",function() {
// Test.it("Basic tests: variance, data",function() {  
